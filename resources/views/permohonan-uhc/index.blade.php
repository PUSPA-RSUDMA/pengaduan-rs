                    ...
                    <table class="table table-hover table-striped align-middle small">
                        <thead class="table-dark">
                            <tr>
                                ...
                                <th>Pasien</th>
                                <th>No. HP</th>
                                <th>Segmen Sebelumnya</th>
                                <th>Alasan Peralihan</th>
                        ...
                        <tbody>
                            @forelse($data as $item)
                                <tr>
                                    ...
                                    <td><span class="badge bg-info text-dark">{{ $item->segmen_kepesertaan }}</span></td>
                                    <td>{{ Str::limit($item->alasan_peralihan, 40) }}</td>
                                    <td class="text-center">
                                        ...
                                        <a href="{{ Storage::disk('google')->url($item->file_lampiran) }}" target="_blank" class="btn btn-sm btn-outline-info rounded-3">
                                            <i class="bi bi-cloud-arrow-down"></i> Lihat
                                        </a>
                                        @else
                                    ...
                                    <td class="text-center">
                                        {{-- Jika Anda belum membuat file edit.blade.php, Anda bisa hapus tombol edit ini sementara --}}
                                        <a href="{{ route('permohonan-uhc.edit', $item->id) }}" class="btn btn-sm btn-warning rounded-3 shadow-sm" title="Edit">
                                            <i class="bi bi-pencil-square"></i>
                                        </a>
                                        <form action="{{ route('permohonan-uhc.destroy', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini? File lampiran juga akan terhapus permanen.')">
                                            @csrf
                                            @method('DELETE')
            ...
            <form action="{{ route('permohonan-uhc.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body p-4">
                    <div class="row g-3">
                        ...
                        <div class="col-md-12">
                            <label class="form-label fw-semibold small">Segmen Kepesertaan Sebelumnya <span class="text-danger">*</span></label>
                            <input type="text" name="segmen_kepesertaan" class="form-control" required placeholder="Contoh: Mandiri / PBPU / PBI APBN">
                        </div>
                        <div class="col-md-12">
                            <label class="form-label fw-semibold small">Alasan Peralihan <span class="text-danger">*</span></label>
                            <textarea name="alasan_peralihan" class="form-control" rows="3" required placeholder="Jelaskan alasan beralih ke UHC..."></textarea>
                        </div>
                        <div class="col-md-12">
                            ...
                            <div class="form-text small">Format file: PDF, JPG, PNG. Maksimal ukuran: 10MB.</div>
                        </div>
                    </div>
                </div>
